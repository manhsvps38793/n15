...
?>
    ...
    <div class="form-signup clearfix">
        <fieldset class="form-group">
            ...
            <input type="password" name="mkc" id="OldPass" required class="form-control form-control-lg">
        </fieldset>
        <fieldset class="form-group">
            <label for="changePass">Mật khẩu mới <span class="error">*</span></label>
            <input type="password" name="mkm" id="changePass" required class="form-control form-control-lg">
        </fieldset>
        <fieldset class="form-group">
        ...
        <button class="button btn-edit-addr btn btn-primary btn-more" type="submit"><i class="hoverButton"></i>Đặt lại mật khẩu</button>
    </div>
</form>
							</div>
						</div>
	...
	<?php
					...
					foreach ($result as $x) {
						if ($mkc == $x['MatKhau']) {
							$sql = "UPDATE taikhoan SET MatKhau = :mkm WHERE MatKhau = :mkc";
							$db = new ConnectModel();
							$db->ketnoi(); 
							$stmt = $db->conn->prepare($sql);
							$stmt->bindParam(':mkc', $mkc);
							$stmt->bindParam(':mkm', $mkm);
							$stmt->execute();
							$db->conn = null; 
						}
					}
